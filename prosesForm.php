...
if (isset($_POST['book'])) {
    ...
    // MODIFIKASI DAN MANIPULASI DATA HOTEL UNTUK ID HOTEL
    if ($hotel === "1") {
        $id_hotel = 2;
        $nama_hotel = "Jiwa Jawa Bromo Hotel";
    } else if ($hotel === "2") {
        $id_hotel = 3;
        $nama_hotel = "Mercure Jakarta Hotel";
    } else if ($hotel === "3") {
        $id_hotel = 4;
        $nama_hotel = "Dedanau Hotel";
    } else if ($hotel === "4") {
        $id_hotel = 5;
        $nama_hotel = "Pesona Bamboe Hotel";
    }
    ...
    VALUES (NULL, '$nama_hotel', '$nama', '$phone', '$type_room', '$tamu', '$checkin', '$checkout', '$jml_malam', '$price', '$id_login', '$id_hotel');";
